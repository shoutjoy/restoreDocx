...
**[https://shoutjoy.github.io/restoreDocx/](https://shoutjoy.github.io/restoreDocx/)**

GitHub Pages에 배포된 주소입니다. 별도 설치 없이 브라우저에서 파일을 업로드하면 바로 복구를 시도할 수 있습니다.

---

## 소개

연세대학교 심리학과에서 **과제 제출**, 보고서 작성, 실습 기록 등 업무 과정에서 MS Word 파일이 깨져 Word로 열리지 않는 경우가 종종 발생합니다. 저장 실수, 비정상 종료, 다른 프로그램으로 덮어쓰기, 클라우드 동기화 오류 등으로 `.docx`의 ZIP 구조나 `word/document.xml`이 손상되면 학생·교수 모두 당황하기 쉽습니다.

이 앱은 **현장(강의실·연구실·지도 시간)** 에서 즉시 본문 텍스트를 뽑아 내고, 필요하면 최소 형태의 DOCX로 다시 저장할 수 있도록 만든 **현장 복구용 웹 도구**입니다. 서버로 파일을 보내지 않고 브라우저 안에서만 처리하므로, 제출물·개인 데이터를 다룰 때도 비교적 안전하게 사용할 수 있습니다.
...
1. 위 **[배포 사이트](https://shoutjoy.github.io/restoreDocx/)** 를 연다. (또는 로컬에서 `index.html` 실행)
2. 손상된 `.docx`(또는 `.zip`, XML이 보이는 `.txt`)를 드래그하거나 선택한다.
3. **복원된 텍스트** 탭에서 내용을 확인한다.
4. **텍스트 복사** 또는 **DOCX 다운로드**로 저장한다.
...
|------|------|
| 1 | 원본 **백업** 복사 |
| 2 | [배포 사이트](https://shoutjoy.github.io/restoreDocx/) 또는 `index.html`에서 파일 업로드 |
